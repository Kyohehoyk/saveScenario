...
public class ScenarioEditServiceImpl implements ScenarioEditService {
	...
	public void init(ScenarioEditDto scenarioEditDto) {
		...
		cond.setId(Integer.valueOf(scenarioEditDto.getSessionId()));
		List<ListsDto> dto = this.listsService.Select(cond);
		ListsDto listsDto = dto.get(0);
	...
	private void insertOrUpdateScenarioInfos(String kind, String content, Integer sessionId) {
		...
		if (CollectionUtils.isEmpty(select)) {
			...
			this.scenarioInfosService.update(keyScenarioInfosDto);
		}
	}
